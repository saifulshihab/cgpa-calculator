...
const cgpaCalculate = () => {
  ...
  let totalCreditnSgpa = 0, totalCredit = 0;
  for (let i = 0; i < sgpa.length; i++) {
    if (sgpa[i].value == "" || credit[i].value == "") {
      ...
      totalCreditnSgpa += parseFloat(sgpa[i].value) * parseFloat(credit[i].value);
      totalCredit += parseFloat(credit[i].value);
      document.getElementById("result").innerHTML =
        "Your CGPA : " + parseFloat(totalCreditnSgpa / totalCredit).toFixed(2);
    }
  }
};
